...
const WORK_SELECT_FIELDS = [
  "id", "title", "display_name", "doi", "publication_year", "publication_date",
  "cited_by_count", "is_retracted", "authorships", "primary_topic", "topics",
  "keywords", "referenced_works", "related_works", "abstract_inverted_index",
].join(",");

const nonNegativeHeaderNumber = (headers: Headers, name: string): number | null => {
...
const retryAfterMilliseconds = (headers: Headers, fallbackMs: number, now = Date.now()): number => {
...
export class OpenAlexProvider implements ScholarlyProvider {
  ...
  #fallbackUsage: RequestUsage = {
    requestCount: 0, cacheHitCount: 0, creditsUsed: 0, rateLimitRemaining: null, creditEstimateDelta: 0, estimatedCostUsd: 0,
  };
  ...
  public async searchWorks(input: ProviderSearchInput, signal?: AbortSignal, deadlineAt?: number): Promise<SearchResult> {
  ...
  public async getWork(identifier: string, signal?: AbortSignal, deadlineAt?: number): Promise<PaperDetail | null> {
    ...
    try {
      return normalizeOpenAlexWork(await this.#request(url, "singleton", signal, deadlineAt));
    } catch (error) {
  ...
  async #request(url: URL, kind: FetchKind, parentSignal?: AbortSignal, deadlineAt?: number): Promise<unknown> {
    ...
    for (let attempt = 0; attempt < 3; attempt += 1) {
      ...
      try {
        ...
        const actualCredits = nonNegativeHeaderNumber(response.headers, "X-RateLimit-Credits-Used");
        if (actualCredits !== null) {
          const delta = actualCredits - credits;
          usage.creditsUsed += delta;
          usage.creditEstimateDelta = Number((usage.creditEstimateDelta + delta).toFixed(6));
        }
        const remaining = nonNegativeHeaderNumber(response.headers, "X-RateLimit-Remaining");
        if (remaining !== null) usage.rateLimitRemaining = remaining;
        ...
        if (response.status === 429) {
          const resetSeconds = nonNegativeHeaderNumber(response.headers, "X-RateLimit-Reset");
          const retryAfterMs = retryAfterMilliseconds(response.headers, resetSeconds !== null ? resetSeconds * 1_000 : 250 * 2 ** attempt);
          if (remaining === 0) {
            throw new AppError("PROVIDER_RATE_LIMIT", "OpenAlex 일일 예산이 소진되어 현재 호출에서는 재시도하지 않습니다.", {
              retryable: true,
              details: { retryAfterMs },
            });
          }
          if (attempt < 2) {
            await this.#waitForRetry(retryAfterMs, "PROVIDER_RATE_LIMIT", parentSignal, deadlineAt);
            continue;
          }
          throw new AppError("PROVIDER_RATE_LIMIT", "OpenAlex 요청 한도를 초과했습니다.", { retryable: true });
        }
        if (response.status >= 500 && attempt < 2) {
          const retryAfterMs = retryAfterMilliseconds(response.headers, 250 * 2 ** attempt);
          await this.#waitForRetry(retryAfterMs, "PROVIDER_TIMEOUT", parentSignal, deadlineAt);
          continue;
        }
        if (!response.ok) {
        ...
        if (attempt < 2) {
          await this.#waitForRetry(250 * 2 ** attempt, "PROVIDER_TIMEOUT", parentSignal, deadlineAt);
          continue;
        }
      }
    }
  ...
  async #waitForRetry(
    retryAfterMs: number,
    code: "PROVIDER_RATE_LIMIT" | "PROVIDER_TIMEOUT",
    signal?: AbortSignal,
    deadlineAt?: number,
  ): Promise<void> {
    if (deadlineAt !== undefined && Date.now() + retryAfterMs + this.config.requestTimeoutMs > deadlineAt) {
      throw new AppError(code, "남은 도구 실행 시간 안에 제공자 대기와 다음 요청을 완료할 수 없어 재시도를 중단했습니다.", {
        retryable: true,
        details: { retryAfterMs },
      });
    }
    try {
      ...
      throw new AppError("PROVIDER_TIMEOUT", "OpenAlex 재시도 대기가 취소됐습니다.", {
        cause: error,
        retryable: true,
        details: { retryAfterMs },
      });
...
export const openAlexProviderInternals = { nonNegativeHeaderNumber, retryAfterMilliseconds };
