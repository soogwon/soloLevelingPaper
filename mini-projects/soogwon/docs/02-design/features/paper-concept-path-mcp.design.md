...
제목 검색 1위가 다음 후보와 충분히 구분되지 않으면 `ambiguous`로 반환하고 자동 선택하지 않는다. 정확한 구분 기준은 정규화 제목 일치 여부와 제공자 관련도 차이를 함께 사용한다.
...
- 제공자의 응답 비용 또는 요청 한도 헤더가 있으면 디버그 로그와 결과 통계에 반영한다.
