...
`npm run check`는 타입 검사, 자동 테스트 및 빌드를 순서대로 실행합니다.
...
응답의 `paths`에는 점수가 높은 순서로 최대 3개 경로가 포함됩니다. 유효한 경로가 없으면 빈 배열을 반환합니다.

> **응답 계약 변경:** 개발 중 단일 `path: object | null` 응답을 `paths: object[]`로 변경했습니다. 기존 소비자는 `path` 존재 여부 대신 `paths.length > 0`을 확인해야 합니다. 대표 경로는 `paths[0]`이며 경로가 없으면 `paths`는 빈 배열입니다.
...
외부 요청 하나의 타임아웃은 기본 5초이며, 논문 검색·확인 도구의 전체 실행 제한은 10초, 경로 탐색은 8초입니다. 429 또는 일시적 5xx의 `Retry-After`는 숫자 초와 HTTP 날짜 형식을 모두 지원합니다. 서버가 요구한 대기와 다음 요청을 전체 실행 제한 안에 완료할 수 있을 때만 재시도하며, 시간이 부족하면 대기 시간을 임의로 줄여 조기 재시도하지 않습니다. 일일 예산이 소진된 429는 현재 호출 안에서 재시도하지 않습니다.
...
- 첫 버전은 점수순 최대 3개 경로를 반환하며 그래프 UI와 PDF 전문 분석은 제공하지 않습니다.
