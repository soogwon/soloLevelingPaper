...
def register_and_ingest_url(
    ...
    PDF 링크면 register_and_ingest와 동일한 파이프라인을 그대로 재사용하고,
    일반 웹페이지면 본문을 추출해 "페이지 1개"로 취급한다.

    주의: HTML로 등록된 논문은 pdf_page가 항상 1이고 printed_page_label도
    항상 None이다 — 실제 인쇄 페이지 개념이 없기 때문이다. 이 점은 ⑩(페이지
    근거 표시) 기능에서 HTML 출처와 PDF 출처를 구분해 안내해야 할 수 있다.

    tmp_pdf_dir: PDF로 판별된 경우 다운로드한 바이트를 저장할 임시 폴더.
    실제로는 이것도 register_and_ingest의 "PDF 실제 복사(TODO)"와 함께
    runtime/pdfs 관리 정책에 맞춰 정리해야 한다 — C 영역과 합의 필요.
    """
    from solo_leveling.infrastructure.parsing.url_ingest import fetch_and_classify
    ...
    if kind == "pdf":
        ...
        result = register_and_ingest(
            ...
            chroma_dir,
            str(tmp_path),
            original_filename=Path(url).name or "downloaded.pdf",
            paper_id=paper_id,
    ...
    from solo_leveling.infrastructure.parsing.url_ingest import extract_html_text
    ...
    conn.close()
    if not exists:
        repo.insert_paper(db_path, Paper(paper_id=paper_id, title=None, source_kind="url"))
    ...
    version = PaperVersion(
        ...
        paper_id=paper_id,
        file_hash=file_hash,
        stored_path=url,  # HTML은 파일이 아니라 URL 자체가 출처
    ...
    try:
        repo.update_job_status(db_path, job.job_id, JobStatus.PROCESSING, stage=JobStage.PARSE)
        # HTML은 실제 페이지 개념이 없어 페이지 1개로 취급 (printed_page_label/pdf_page 무의미)
        page = ExtractedPage(pdf_page=1, text=text, printed_page_label=None)
        parse_revision = ParseRevision(
            parse_revision_id=str(uuid.uuid4()), version_id=version.version_id, page_count=1
        )
        repo.insert_parse_revision(db_path, parse_revision)

        chunks = chunk_pages(parse_revision.parse_revision_id, [page])
        if chunks:
            repo.insert_chunks(db_path, chunks)

        repo.update_job_status(db_path, job.job_id, JobStatus.PROCESSING, stage=JobStage.INDEX)
        limitations = ["html_source_no_real_pagination"]
        if not chunks:
            limitations.append("extraction_empty")

        if chunks:
            texts_to_embed = [c.original_text for c in chunks]
            vectors = embed_texts(texts_to_embed, model_name=embedding_model)
            embedding_set = EmbeddingSet(
                embedding_set_id=str(uuid.uuid4()),
                translation_revision_id=str(uuid.uuid4()),  # 임시 통과, register_and_ingest와 동일한 한계
                model_name=embedding_model,
        ...
        repo.update_job_status(db_path, job.job_id, JobStatus.FAILED, limitations=[f"error: {e}"])
        raise
